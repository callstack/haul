...
function devToolsMiddleware(debuggerProxy) {
  return (req, res, next) => {
    ...
    switch (req.url) {
       ...
       */
      case '/debugger-ui': {
        const readStream = fs.createReadStream(path.join(__dirname, '../assets/debugger.html'));
      ...
      case '/debuggerWorker.js': {
        const readStream = fs.createReadStream(path.join(__dirname, '../assets/debuggerWorker.js'));
      ...
      case '/launch-js-devtools': {
        if (!debuggerProxy.isChromeConnected()) {
