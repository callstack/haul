...
* `entry: string | string[]` (__required__) - Path to entry file(s).
...
* `dll?: boolean` (_optional_, __multi-bundle mode only__)
* `app?: boolean` (_optional_, __multi-bundle mode only__)
* `dependsOn?: string[]` (_optional_, __multi-bundle mode only__)
...
* `transform?: WebpackConfigTransform` - Customize the Webpack config after it's been created.
  ...
  transform: ({ bundleName, env, runtime, config }) => {
    runtime.logger.info(`Altering Webpack config for bundle ${bundleName} for ${env.platform}`);
    // alter the config ...
  }
  ```
